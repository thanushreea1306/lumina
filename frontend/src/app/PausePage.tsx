/* ============================================================
   LUMINA PAUSE Intervention
   ============================================================
   Calm, protective, focused interruption.
   "Pause before you act."
   NOT alarmist, flashing, aggressive, or hacker-like.
   ============================================================ */
...
function getPauseGuidance(reasonCodes: string[]) {
  const items: Array<{ text: string; type: 'safe' | 'warn' }> = [];

  items.push({ text: 'Stop what you are doing and take a moment.', type: 'safe' });
  items.push({ text: 'Do not share any codes, passwords, or financial information.', type: 'warn' });
  items.push({ text: 'Do not make any payments or transfers.', type: 'warn' });
  items.push({ text: 'You can end this interaction at any time.', type: 'safe' });
  ...
  if (reasonCodes.some((r) => r.includes('money') || r.includes('payment') || r.includes('transfer'))) {
    items.push({ text: 'Do not send money or make payments under pressure.', type: 'warn' });
  }
  if (reasonCodes.some((r) => r.includes('threat') || r.includes('arrest') || r.includes('legal'))) {
    items.push({ text: 'Legitimate authorities do not demand immediate payment or secrecy.', type: 'safe' });
  }

  return items;
}

// ---- PAUSE Response Options ----
const PAUSE_RESPONSES = [
  {
    action: 'pause_decision',
    response: 'stopped_interaction',
    label: 'I stopped the interaction',
    description: 'You ended or paused the potentially risky interaction.',
    icon: '⏸',
  ...
  },
  {
    ...
    description: 'LUMINA cannot block this action automatically. You are choosing to continue.',
    icon: '→',
...
export function PausePage() {
  ...
  if (session.status === 'loading' || session.status === 'creating') {
    return (
      <InterventionShell state="PAUSE">
        <LoadingState message="Loading pause status" />
      </InterventionShell>
    );
  }
    ...
    return (
      <InterventionShell state="PAUSE">
        <div className="no-session-banner">
          <div className="intervention-icon" data-state="PAUSE" aria-hidden="true" style={{ margin: '0 auto var(--space-4)' }}>⏸</div>
          <h2>No Active Safety Session</h2>
          <p>Start a session from the Session page to access the Pause intervention.</p>
  ...
  const guidance = getPauseGuidance(decision.reasonCodes);
  ...
  return (
    <InterventionShell state="PAUSE">
      <InterventionHeader
        ...
        icon="⏸"
        title="Pause"
        subtitle="Something in this interaction requires caution. Stop and review before taking any action."
      />
      ...
      {/* Reason Codes */}
      <ReasonList reasons={decision.reasonCodes} state="PAUSE" />
      ...
      {/* Safety Guidance */}
      <SafetyGuidance title="Protect Yourself" items={guidance} />
      ...
      <div className="capability-note" style={{ marginBottom: 'var(--space-4)' }}>
        ...
        It provides evidence and guidance to help you make informed decisions.
      </div>
      ...
      <div style={{ marginBottom: 'var(--space-6)' }}>
        ...
        {submittedResponse ? (
          <div className="response-submitted" role="status" aria-live="polite">
            ...
            <span>
              Response recorded: {submittedResponse === 'stopped_interaction' ? 'Interaction Stopped' :
